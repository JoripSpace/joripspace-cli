...
const { resolveProjectRoot, startProject, createStartOnboarding } = require('./start');
  ...
  return async function startProjectCommand(rest, flags) {
    ...
    if (!connection?.apiToken) {
      const apiUrlArgument = connection?.apiBaseUrl
        ...
        : '';
      const invocation = `${executable === path.resolve(process.argv[1] || '') ? quoteCliArgument(process.execPath) + ' ' : ''}${quoteCliArgument(executable)}`;
      ...
      const value = {
        ...
        continue_with: 'cli',
        onboarding: createStartOnboarding(executable, cwd, project, false, connection?.apiBaseUrl),
      };
    ...
    if (!value.ok) {
      value.onboarding = createStartOnboarding(executable, cwd, project, false, connection.apiBaseUrl);
    ...
    output(flags, value, (result) => {
      ...
      console.log('Read the project instructions now, even if your agent does not automatically load them:');
      for (const file of result.onboarding.read_files) console.log(file);
      console.log('Use --json for the inline guide and executable/argument objects for this session.');

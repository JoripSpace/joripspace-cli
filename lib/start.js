...
const { projectAgentsFile, JORIPSPACE_CLI_GUIDE } = require('../vendor/core/onboarding.cjs');
...
async function startProject(options) {
  ...
  const sourceSyncBlocking =
    ...
    ![
      ...
      'empty_remote_ready',
      'initialized_empty_remote',
    ].includes(sourceSync.status);
  ...
  if (
    deploymentMode === 'github_actions' &&
    details?.deployment?.source_status === 'empty' &&
    restorable &&
    !existingWorkBeforeGuidance
  ) {
    sourceSync = {
      status: 'latest_deployment_restore_required',
      deployment_id: latestDeployment.deployment_id,
      repository_setup: sourceSync,
    };
  } else if (!sourceSync && deploymentMode !== 'github_actions') {
    sourceSync =
  ...
  const hasExistingWork = Boolean(
    existingWorkBeforeGuidance ||
      latestDeployment ||
      (details?.deployment?.mode === 'github_actions' && details?.deployment?.source_status !== 'empty')
  );
  const warnings = [];
  ...
  return {
    ...
    executable,
    continue_with: 'cli',
    onboarding: createStartOnboarding(executable, workspace, projectSlug, true),
...
async function loadTemplateChoice(apiRequest, apiToken, executable, projectSlug, workspace) {
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      ...
      return {
        status: 'ready',
        optional: true,
        no_template_option: { number: 0, name: 'Start without a template' },
        ...
        presentation:
          'Before asking project questions, show option 0 and templates in a Markdown table with number, template name, suitable use, and included features, then wait for the user to choose.',
        install_command: `${executable === path.resolve(process.argv[1] || '') ? quoteCommandArgument(process.execPath) + ' ' : ''}${quoteCommandArgument(executable)} install-template --template "TEMPLATE_SLUG" --project ${quoteCommandArgument(projectSlug)} --dir ${quoteCommandArgument(workspace)}`,
      };
    } catch {
      if (attempt === 2) break;
    }
  ...
  return {
    ...
    retryable: true,
    next_action:
      'Run start once more with the same absolute CLI path and project. Do not continue to service questions before the template choice is available.',
  };
}
...
function syncConnectedGithubRepository(workspace, deployment, gitCommand, options = {}) {
  ...
  if (deployment.source_status === 'empty') {
    return prepareEmptyGithubRepository(workspace, gitCommand, repository, branch, expectedUrl);
  }
...
function prepareEmptyGithubRepository(workspace, gitCommand, repository, branch, expectedUrl) {
  const inside = git(workspace, gitCommand, ['rev-parse', '--is-inside-work-tree']);
  if (inside.status === 0 && inside.stdout.trim() === 'true') {
    const remote = git(workspace, gitCommand, ['remote', 'get-url', 'origin']);
    if (remote.status !== 0) {
      const addRemote = git(workspace, gitCommand, ['remote', 'add', 'origin', expectedUrl]);
      if (addRemote.status !== 0) return gitFailure('origin_add_failed', addRemote);
    } else if (githubRepository(remote.stdout.trim()) !== repository.toLowerCase()) {
      return {
        status: 'wrong_repository',
        expected_repository: repository,
        actual_remote: redactGitDiagnostic(remote.stdout.trim()),
    ...
    const currentBranch = git(workspace, gitCommand, ['symbolic-ref', '--quiet', '--short', 'HEAD']);
    if (currentBranch.status !== 0) return gitFailure('branch_check_failed', currentBranch);
    if (currentBranch.stdout.trim() !== branch) {
      const head = git(workspace, gitCommand, ['rev-parse', '--verify', 'HEAD']);
      if (head.status === 0) {
        return {
          status: 'wrong_branch',
          expected_branch: branch,
          actual_branch: currentBranch.stdout.trim(),
        };
      }
      const setBranch = git(workspace, gitCommand, ['symbolic-ref', 'HEAD', `refs/heads/${branch}`]);
      if (setBranch.status !== 0) return gitFailure('branch_setup_failed', setBranch);
    }
    return { status: 'empty_remote_ready', operation: 'existing_repository', repository, branch };
  }

  const initialize = git(workspace, gitCommand, ['init', '--initial-branch', branch]);
  if (initialize.status !== 0) return gitFailure('init_failed', initialize);
  const addRemote = git(workspace, gitCommand, ['remote', 'add', 'origin', expectedUrl]);
  if (addRemote.status !== 0) return gitFailure('origin_add_failed', addRemote);
  return { status: 'initialized_empty_remote', operation: 'git_init', repository, branch };
}
...
function createStartOnboarding(executable, cwd, project, connected = false, apiBaseUrl = '') {
  const invocation = {
    command: process.pkg ? executable : process.execPath,
    args: process.pkg ? [] : [executable],
    cwd,
  };
  const command = (args) => ({
    command: invocation.command,
    args: [...invocation.args, ...args, ...(apiBaseUrl ? ['--api-url', apiBaseUrl] : [])],
    cwd,
  });
  return {
    schema_version: 1,
    instructions: JORIPSPACE_CLI_GUIDE,
    read_files: connected ? [path.join(cwd, 'AGENTS.md'), path.join(cwd, 'CLAUDE.md')] : [],
    invocation,
    commands: {
      resume: command(['start', project, '--cwd', cwd, '--json']),
      help: command(['--help']),
      ...(!connected ? { login: command(['login', '--code', 'CONNECTION_CODE', '--cwd', cwd, '--json']) } : {}),
    },
  };
}

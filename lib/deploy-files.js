...
function buildInlineDeployPayload(entrypoint, sources) {
  ...
  assertNoUnresolvedPackageImports(files);
  return { entrypoint, files };
}

function assertNoUnresolvedPackageImports(files) {
  const unresolved = findUnresolvedPackageImports(files);
  if (!unresolved.length) return;
  const error = new Error(
    `package imports are not bundled: ${unresolved.join(', ')}. Run the project build and deploy the generated Worker entrypoint (for example dist/worker.js).`
  );
  error.code = 'unbundled_dependency';
  throw error;
}

function assertNoUnresolvedPackageImportsInDirectory(root) {
  const files = {};
  for (const file of walkFiles(root)) {
    if (!/\.(?:[cm]?[jt]sx?)$/i.test(file.path)) continue;
    const relative = safeProjectRelativePath(toPosix(path.relative(root, file.path)));
    files[relative] = fs.readFileSync(file.path, 'utf8');
  }
  assertNoUnresolvedPackageImports(files);
}

function findUnresolvedPackageImports(files) {
  const fileNames = new Set(Object.keys(files).map((name) => name.replace(/\\/g, '/')));

...
동작 변경은 내부 require 경로 두 곳과 Node 실행에 맞춘 `start` 후속 명령 문자열뿐이다. 명령·옵션·API·토큰 검증·설정 저장·JSON 키·종료 코드·작업 폴더 판단은 유지한다. Windows에서도 후속 명령을 실행할 수 있도록 기존 문자열에 Node 실행 경로를 추가했다.
...
이번 작업에서는 npm publish, GitHub push, release 생성, 운영 플랫폼 배포를 실행하지 않는다.
